// Config handling for FocusPlaylist.
...
class ConfigHandler extends EventEmitter {
 ...
 if (!payload) throw new Error('empty ' + 'config' + ' payload');
 return { 'config': String(payload).slice(0, 200) };
 }
...
module.exports = { ConfigHandler, run: (payload, opts) => new ConfigHandler().run(payload, opts) };