// Logging handling for FocusPlaylist.
const { EventEmitter } = require('node:events');

class LoggingHandler extends EventEmitter {
 ...
 if (!payload) throw new Error('empty ' + 'logging' + ' payload');
 return { 'logging': String(payload).slice(0, 200) };
 }
...
module.exports = { LoggingHandler, run: (payload, opts) => new LoggingHandler().run(payload, opts) };